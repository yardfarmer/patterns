...
由于 `for in` 循环会枚举 原型链 上的所有属性，唯一过滤这些属性的方式是使用 [`hasOwnProperty`](#object.hasownproperty) 函数，
因此会比普通的 `for` 循环慢上好多倍。
...
并且作为数组的一大特性，length 值是跟随元素个数变动的。类数组中是没有这个特性的。
	

单纯的遍历对象自身的属性，可以：
	
	var Arr = {a: 1, b: 2};
	var objArr = Object.keys(A);
	Array.isArray(objArr); //  => true

通过 Object.keys 返回属性名数组.
