---
layout: post
title: "javascript 设计模式之对象创建"
date: "2015-03-25 10:55"
...
在JavaScript中创建对象很容易——可以通过使用对象直接量或者构造函数。

JavaScript语言本身简单、直观，通常也没有其他语言那样的语法特性：命名空间、模块、包、私有属性以及静态成员。
...
这种模式是一种很好的提供命名空间的方式，避免了自身代码的命名冲突，同时还避免了同一个页面上自身代码和第三方代码的冲突。这种模式在大多数情况下非常适用，但也有它的缺点：
...
随着程序复杂度的提高，代码会分置在不同的文件中以特定顺序来加载，或者是现在采用当前比较流行的异步加载,这样一来，就不能保证你的代码一定是第一个声明命名空间或者改变量下的属性的。甚至还会发生属性覆盖的问题。所以，在创建命名空间或者添加属性的时候，最好先检查下是否存在，如下所示：
```javascript
	// unsafe
	var MYAPP = {};
	// better
...
如上所示，不难看出，如果每次做类似操作都要这样检查一下就会有很多重复性的代码。比方说，要申明`MYAPP.modules.module2`，就要重复三次这样的检查。所以，我们需要一个重用的`namespace()`函数来专门处理这些检查工作，然后用它来创建命名间，如下所示:
...
还有一个原理一样但看起来不一样的实现示例：
	...
	myobj.getName(); // "my, oh my"
```
这个例子也是所谓的“模块模式”的基础. 其实使用**闭包**的本质没有变, 而闭包具有这种作用的基础是 js 使用的是_词法作用域_, 靠的是分析代码的位置, 而不是runtime.
...
模块模式也可以用类似的方法来组织，将所有的方法保持私有，只在最后暴露需要使用的方法来初始化API。
...
```javascript
	...
	// AMD/CMD
	// define('hello', ['./mod-01'], function(require, exports, module) {
	//
	//     // 模块代码
	//
	// });
...
使用通配符星号来表示“使用所有可用的模块”如何？为了方便，我们也假设没有任何模块传入时，沙箱使用星号。所以有两种使用所有可用模块的方法：
```javascript
